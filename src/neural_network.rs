...
impl NeuralNetwork {
    pub fn new(dims: &[usize]) -> NeuralNetwork {
        if dims.len() < 2 {
            panic!(format!(
...
fn generate_random_weights(dims: &[usize]) -> Vec<Vec<Vec<f32>>> {
    let rng = rand::thread_rng();

    let mut random_weights = vec![];
...
fn generate_random_biases(dims: &[usize]) -> Vec<Vec<f32>> {
    let rng = rand::thread_rng();

    let mut random_biases = vec![];
